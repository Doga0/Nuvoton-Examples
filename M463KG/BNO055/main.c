...
float verticalVelocity(float *acc, float pitch, float roll){
	float AccZ_Inertial = 0;
	float vertical_v = 0;
...
int main(){
	...
	while(1){
		...
		float *acc = BNO055_readAcc(milis, 0);
		//printf("Acc x: %.2f\n", acc[0]/100);
		//printf("Acc y: %.2f\n", acc[1]/100);
		//printf("Acc z: %.2f\n", acc[2]/100);
		...
		//double offsetAccz = offsetAccZ();
		//printf("offset: %f\n", offsetAccz);
				
		float pitch = pitchAngle(acc);
		//printf("pitch:%f\n", pitch);
				
		float roll = rollAngle(acc);
		//printf("roll: %f\n", roll);
